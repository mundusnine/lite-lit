...
static int f_window_has_focus(lua_State *L) {
  ...
  lua_pushboolean(L, 1/*flags & SDL_WINDOW_INPUT_FOCUS*/);
  return 1;
}
...
static int f_get_clipboard(lua_State *L) {
  char *text = NULL;//SDL_GetClipboardText();
  if (!text) { return 0; }
...
static int f_get_time(lua_State *L) {
  double n = fenster_time();// SDL_GetPerformanceCounter() / (double) SDL_GetPerformanceFrequency();
  lua_pushnumber(L, n);
  return 1;
}
...
static int f_update(lua_State* L) {
    lua_pushboolean(L, fenster_loop(f));
    return 1;
}
